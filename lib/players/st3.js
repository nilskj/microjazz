...
function Screamtracker() {
    ...
    this.effects_t0 = new Array(function (mod, ch) { }, // zero is ignored
    ...
    this.effects_t0_s = new Array(this.effect_t0_s0, this.effect_t0_s1, this.effect_t0_s2, this.effect_t0_s3, this.effect_t0_s4, this.effect_t0_s5, this.effect_t0_s6, this.effect_t0_s7, this.effect_t0_s8, this.effect_t0_s9, this.effect_t0_sa, this.effect_t0_sb, this.effect_t0_sc, this.effect_t0_sd, this.effect_t0_se, this.effect_t0_sf);
    this.effects_t1 = new Array(function (mod, ch) { }, // zero is ignored
    this.effect_t1_a, this.effect_t1_b, this.effect_t1_c, this.effect_t1_d, this.effect_t1_e, this.effect_t1_f, this.effect_t1_g, this.effect_t1_h, this.effect_t1_i, this.effect_t1_j, this.effect_t1_k, this.effect_t1_l, this.effect_t1_m, this.effect_t1_n, this.effect_t1_o, this.effect_t1_p, this.effect_t1_q, this.effect_t1_r, this.effect_t1_s, this.effect_t1_t, this.effect_t1_u, this.effect_t1_v, this.effect_t1_w, this.effect_t1_x, this.effect_t1_y, this.effect_t1_z);
    this.effects_t1_s = new Array(this.effect_t1_s0, this.effect_t1_s1, this.effect_t1_s2, this.effect_t1_s3, this.effect_t1_s4, this.effect_t1_s5, this.effect_t1_s6, this.effect_t1_s7, this.effect_t1_s8, this.effect_t1_s9, this.effect_t1_sa, this.effect_t1_sb, this.effect_t1_sc, this.effect_t1_sd, this.effect_t1_se, this.effect_t1_sf);
}
exports.default = Screamtracker;
// clear song data
...
Screamtracker.prototype.parse = function (buffer) {
    ...
    for (i = 0; i < 4; i++)
        this.signature += String.fromCharCode(buffer[0x002c + i]);
    if (this.signature != "SCRM")
        return false;
    if (buffer[0x001d] != 0x10)
        return false;
    // get channel count
    ...
    while (buffer[i] && i < 0x1c)
        this.title += (0, utils_1.dos2utf)(buffer[i++]);
    ...
    // load orders
    for (i = 0; i < this.ordNum; i++)
        this.patterntable[i] = buffer[0x0060 + i];
    for (this.songlen = 0, i = 0; i < this.ordNum; i++)
        if (this.patterntable[i] != 255)
            this.songlen++;
    // load instruments
    ...
    for (i = 0; i < this.insNum; i++) {
        ...
        while (buffer[offset + 0x0030 + j] && j < 28) {
            this.sample[i].name += (0, utils_1.dos2utf)(buffer[offset + 0x0030 + j]);
            j++;
        }
        ...
        for (j = 0; j < this.sample[i].length; j++)
            this.sample[i].data[j] = (buffer[smpoffset + j] - 128) / 128.0; // convert to mono float signed
    }
    // load and unpack patterns
    var max_ch = 0;
    this.pattern = new Array();
    for (i = 0; i < this.patNum; i++) {
        var offset = (buffer[0x0060 + this.ordNum + this.insNum * 2 + i * 2] |
            (buffer[0x0060 + this.ordNum + this.insNum * 2 + i * 2 + 1] << 8)) *
            16;
        ...
        if (!offset)
            continue; // fix for control_e.s3m
        row = 0;
        ch = 0;
        offset += 2;
        while (row < 64) {
            if ((c = buffer[offset + pos++])) {
                ...
                if (ch < this.channels) {
                    ...
                    if (c & 128) {
                        ...
                        if (!this.pattern[i][row * this.channels * 5 + ch * 5 + 3] ||
                            this.pattern[i][row * this.channels * 5 + ch * 5 + 3] > 26) {
                            this.pattern[i][row * this.channels * 5 + ch * 5 + 3] = 255;
                        }
                    }
                }
...
Screamtracker.prototype.advance = function (mod) {
    ...
    if (mod.row >= 64) {
        ...
        mod.row = 0;
        mod.flags |= 4;
        while (mod.patterntable[mod.position] == 254)
            mod.position++; // skip markers
    }
    // end of song?
...
Screamtracker.prototype.process_tick = function (mod) {
    ...
    for (var ch = 0; ch < mod.channels; ch++) {
        ...
        if (mod.flags & 2) {
            ...
            if (!(mod.channel[ch].command == 0x13 && (mod.channel[ch].data & 0xf0) == 0xd0)) {
                // note delay?
...
//
Screamtracker.prototype.effect_t0_a = function (mod, ch) {
    // set speed
    if (mod.channel[ch].data > 0)
        mod.speed = mod.channel[ch].data;
};
Screamtracker.prototype.effect_t0_b = function (mod, ch) {
    // pattern jump
    mod.breakrow = 0;
    mod.patternjump = mod.channel[ch].data;
    mod.flags |= 16;
};
Screamtracker.prototype.effect_t0_c = function (mod, ch) {
    // pattern break
...
Screamtracker.prototype.effect_t0_d = function (mod, ch) {
    ...
    if ((mod.channel[ch].volslide & 0x0f) == 0x0f) {
        // DxF fine up
        mod.channel[ch].voicevolume += mod.channel[ch].volslide >> 4;
    }
    else if (mod.channel[ch].volslide >> 4 == 0x0f) {
        // DFx fine down
...
Screamtracker.prototype.effect_t0_e = function (mod, ch) {
    ...
    if (mod.channel[ch].voiceperiod > 27392)
        mod.channel[ch].noteon = 0;
...
Screamtracker.prototype.effect_t0_f = function (mod, ch) {
    ...
    if (mod.channel[ch].voiceperiod < 56)
        mod.channel[ch].noteon = 0;
    mod.channel[ch].flags |= 3; // recalc speed
};
Screamtracker.prototype.effect_t0_g = function (mod, ch) {
    // slide to note
...
Screamtracker.prototype.effect_t0_i = function (mod, ch) {
    // tremor
};
Screamtracker.prototype.effect_t0_j = function (mod, ch) {
    // arpeggio
    if (mod.channel[ch].data)
...
Screamtracker.prototype.effect_t0_k = function (mod, ch) {
    // vibrato + volslide
    mod.effect_t0_d(mod, ch);
};
Screamtracker.prototype.effect_t0_l = function (mod, ch) {
    // slide to note + volslide
    mod.effect_t0_d(mod, ch);
};
...
Screamtracker.prototype.effect_t0_o = function (mod, ch) {
    // set sample offset
    if (mod.channel[ch].data)
        mod.channel[ch].lastoffset = mod.channel[ch].data;
...
Screamtracker.prototype.effect_t0_q = function (mod, ch) {
    // retrig note
    if (mod.channel[ch].data)
        mod.channel[ch].lastretrig = mod.channel[ch].data;
    mod.effect_t1_q(mod, ch); // to retrig also on lines with no note but Qxy command
};
Screamtracker.prototype.effect_t0_r = function (mod, ch) {
    // tremolo
};
Screamtracker.prototype.effect_t0_s = function (mod, ch) {
    // Sxy effects
    var i = (mod.channel[ch].data & 0xf0) >> 4;
    mod.effects_t0_s[i](mod, ch);
};
Screamtracker.prototype.effect_t0_t = function (mod, ch) {
    // set tempo
    if (mod.channel[ch].data > 32)
        mod.bpm = mod.channel[ch].data;
};
Screamtracker.prototype.effect_t0_u = function (mod, ch) {
    // fine vibrato
};
Screamtracker.prototype.effect_t0_v = function (mod, ch) {
    // set global volume
...
Screamtracker.prototype.effect_t0_z = function (mod, ch) {
    // sync for FMOD (was: unused)
    mod.syncqueue.unshift(mod.channel[ch].data & 0x0f);
};
//
// tick 0 special Sxy effect functions
//
Screamtracker.prototype.effect_t0_s0 = function (mod, ch) {
    // set filter (not implemented)
};
Screamtracker.prototype.effect_t0_s1 = function (mod, ch) {
    // set glissando control
};
Screamtracker.prototype.effect_t0_s2 = function (mod, ch) {
    // sync for BASS (was: set finetune)
    mod.syncqueue.unshift(mod.channel[ch].data & 0x0f);
};
Screamtracker.prototype.effect_t0_s3 = function (mod, ch) {
    // set vibrato waveform
    mod.channel[ch].vibratowave = mod.channel[ch].data & 0x07;
};
Screamtracker.prototype.effect_t0_s4 = function (mod, ch) {
...
Screamtracker.prototype.effect_t0_s8 = function (mod, ch) {
...
Screamtracker.prototype.effect_t0_sa = function (mod, ch) {
    // old stereo control (not implemented)
};
...
Screamtracker.prototype.effect_t0_sc = function (mod, ch) {
    // note cut
};
Screamtracker.prototype.effect_t0_sd = function (mod, ch) {
    // note delay
    if (mod.tick == (mod.channel[ch].data & 0x0f)) {
        mod.process_note(mod, mod.patterntable[mod.position], ch);
    }
};
Screamtracker.prototype.effect_t0_se = function (mod, ch) {
    // pattern delay
    mod.patterndelay = mod.channel[ch].data & 0x0f;
    mod.patternwait = 0;
};
Screamtracker.prototype.effect_t0_sf = function (mod, ch) {
    // funkrepeat (not implemented)
};
//
// tick 1+ effect functions
//
Screamtracker.prototype.effect_t1_a = function (mod, ch) {
    // set speed
};
Screamtracker.prototype.effect_t1_b = function (mod, ch) {
    // order jump
};
Screamtracker.prototype.effect_t1_c = function (mod, ch) {
    // jump to row
};
...
Screamtracker.prototype.effect_t1_e = function (mod, ch) {
    // slide down
...
Screamtracker.prototype.effect_t1_f = function (mod, ch) {
    // slide up
    if (mod.channel[ch].slidespeed < 0xe0) {
        mod.channel[ch].voiceperiod -= mod.channel[ch].slidespeed * 4;
    }
...
Screamtracker.prototype.effect_t1_g = function (mod, ch) {
    ...
    else if (mod.channel[ch].voiceperiod > mod.channel[ch].slideto) {
        //    mod.channel[ch].voiceperiod-=4*mod.channel[ch].slidetospeed;
        mod.channel[ch].voiceperiod -= 4 * mod.channel[ch].slidespeed;
        if (mod.channel[ch].voiceperiod < mod.channel[ch].slideto)
            mod.channel[ch].voiceperiod = mod.channel[ch].slideto;
...
Screamtracker.prototype.effect_t1_h = function (mod, ch) {
    ...
    mod.channel[ch].voiceperiod +=
        (mod.vibratotable[mod.channel[ch].vibratowave & 3][mod.channel[ch].vibratopos] * mod.channel[ch].vibratodepth) /
            128;
    if (mod.channel[ch].voiceperiod > 27392)
        mod.channel[ch].voiceperiod = 27392;
    if (mod.channel[ch].voiceperiod < 56)
        mod.channel[ch].voiceperiod = 56;
    mod.channel[ch].flags |= 1;
};
Screamtracker.prototype.effect_t1_i = function (mod, ch) {
    // tremor
};
Screamtracker.prototype.effect_t1_j = function (mod, ch) {
    ...
    if ((mod.tick & 3) == 1)
        n += mod.channel[ch].arpeggio >> 4;
    if ((mod.tick & 3) == 2)
        n += mod.channel[ch].arpeggio & 0x0f;
    mod.channel[ch].voiceperiod = (8363.0 * mod.periodtable[n]) / mod.sample[mod.channel[ch].sample].c2spd;
    mod.channel[ch].flags |= 3; // recalc speed
};
Screamtracker.prototype.effect_t1_k = function (mod, ch) {
    // vibrato + volslide
    mod.effect_t1_h(mod, ch);
    mod.effect_t1_d(mod, ch);
};
Screamtracker.prototype.effect_t1_l = function (mod, ch) {
    // slide to note + volslide
...
Screamtracker.prototype.effect_t1_o = function (mod, ch) {
    // set sample offset
};
...
Screamtracker.prototype.effect_t1_r = function (mod, ch) {
    // tremolo
};
Screamtracker.prototype.effect_t1_s = function (mod, ch) {
    // special effects
    var i = (mod.channel[ch].data & 0xf0) >> 4;
    mod.effects_t1_s[i](mod, ch);
};
Screamtracker.prototype.effect_t1_t = function (mod, ch) {
    // set tempo
};
Screamtracker.prototype.effect_t1_u = function (mod, ch) {
    // fine vibrato
};
Screamtracker.prototype.effect_t1_v = function (mod, ch) {
    // set global volume
};
...
//
Screamtracker.prototype.effect_t1_s0 = function (mod, ch) {
    // set filter (not implemented)
};
Screamtracker.prototype.effect_t1_s1 = function (mod, ch) {
    // set glissando control
};
Screamtracker.prototype.effect_t1_s2 = function (mod, ch) {
    // set finetune
};
Screamtracker.prototype.effect_t1_s3 = function (mod, ch) {
    // set vibrato waveform
};
Screamtracker.prototype.effect_t1_s4 = function (mod, ch) {
    // set tremolo waveform
};
...
Screamtracker.prototype.effect_t1_s8 = function (mod, ch) {
    // set panning position
};
Screamtracker.prototype.effect_t1_s9 = function (mod, ch) {
    // -
};
Screamtracker.prototype.effect_t1_sa = function (mod, ch) {
    // old stereo control (not implemented)
};
Screamtracker.prototype.effect_t1_sb = function (mod, ch) {
    // loop pattern
};
Screamtracker.prototype.effect_t1_sc = function (mod, ch) {
    // note cut
    if (mod.tick == (mod.channel[ch].data & 0x0f)) {
...
Screamtracker.prototype.effect_t1_sd = function (mod, ch) {
    // note delay
    mod.effect_t0_sd(mod, ch);
};
Screamtracker.prototype.effect_t1_se = function (mod, ch) {
    // pattern delay
};
Screamtracker.prototype.effect_t1_sf = function (mod, ch) {
    // funkrepeat (not implemented)
};
