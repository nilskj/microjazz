...
function Fasttracker() {
    ...
    for (i = 0; i < 32; i++)
        this.pan[i] = this.finalpan[i] = 0.5;
    ...
    this.voleffects_t0 = [
        ...
        this.effect_vol_t0_60,
        this.effect_vol_t0_70,
        this.effect_vol_t0_80,
        this.effect_vol_t0_90,
        this.effect_vol_t0_a0,
        this.effect_vol_t0_b0,
        this.effect_vol_t0_c0,
        this.effect_vol_t0_d0,
        this.effect_vol_t0_e0,
    ];
    this.voleffects_t1 = [
        this.effect_vol_t1_f0,
        this.effect_vol_t1_60,
        this.effect_vol_t1_70,
        this.effect_vol_t1_80,
        this.effect_vol_t1_90,
        this.effect_vol_t1_a0,
        this.effect_vol_t1_b0,
        this.effect_vol_t1_c0,
        this.effect_vol_t1_d0,
        this.effect_vol_t1_e0,
    ];
...
Fasttracker.prototype.parse = function (buffer) {
    ...
    for (i = 0; i < 17; i++)
        this.signature += String.fromCharCode(buffer[i]);
    if (this.signature !== "Extended Module: ")
        return false;
    if (buffer[37] !== 0x1a)
        return false;
    this.signature = "X.M.";
    this.trackerversion = (0, utils_1.le_word)(buffer, 58);
    if (this.trackerversion < 0x0104)
        return false; // older versions not currently supported
    // song title
    i = 0;
    while (buffer[i] && i < 20)
        this.title += (0, utils_1.dos2utf)(buffer[17 + i++]);
    offset = 60;
    hdrlen = (0, utils_1.le_dword)(buffer, offset);
    this.songlen = (0, utils_1.le_word)(buffer, offset + 4);
    this.repeatpos = (0, utils_1.le_word)(buffer, offset + 6);
    this.channels = (0, utils_1.le_word)(buffer, offset + 8);
    this.patterns = (0, utils_1.le_word)(buffer, offset + 10);
    this.instruments = (0, utils_1.le_word)(buffer, offset + 12);
    this.amigaperiods = !(0, utils_1.le_word)(buffer, offset + 14) & 1;
    this.initSpeed = (0, utils_1.le_word)(buffer, offset + 16);
    this.initBPM = (0, utils_1.le_word)(buffer, offset + 18);
    var maxpatt = 0;
    for (i = 0; i < 256; i++) {
        this.patterntable[i] = buffer[offset + 20 + i];
        if (this.patterntable[i] > maxpatt)
            maxpatt = this.patterntable[i];
    }
    maxpatt++;
    ...
    for (i = 0; i < maxpatt; i++) {
        ...
        for (var row = 0; row < this.patternlen[i]; row++)
    ...
    while (i < this.patterns) {
        this.patternlen[i] = (0, utils_1.le_word)(buffer, offset + 5);
        this.pattern[i] = new Uint8Array(this.channels * this.patternlen[i] * 5);
        ...
        datalen = (0, utils_1.le_word)(buffer, offset + 7);
        offset += (0, utils_1.le_dword)(buffer, offset); // jump over header
        j = 0;
        k = 0;
        while (j < datalen) {
            c = buffer[offset + j++];
            if (c & 128) {
                ...
                if (c & 1)
                    this.pattern[i][k + 0] = buffer[offset + j++];
                if (c & 2)
        ...
        for (k = 0; k < this.patternlen[i] * this.channels * 5; k += 5) {
            ...
            if (this.pattern[i][k + 3] === 0 && this.pattern[i][k + 4] === 0)
                this.pattern[i][k + 3] = 255;
            // remap volume column setvol to 0x00..0x40, tone porta to 0x50..0x5f and 0xff for nop
            if (this.pattern[i][k + 2] < 0x10) {
                this.pattern[i][k + 2] = 0xff;
            }
            else if (this.pattern[i][k + 2] >= 0x10 && this.pattern[i][k + 2] <= 0x50) {
                this.pattern[i][k + 2] -= 0x10;
            }
            else if (this.pattern[i][k + 2] >= 0xf0)
                this.pattern[i][k + 2] -= 0xa0;
        }
        // unpack next pattern
        offset += j;
        i++;
    ...
    while (i < this.instruments) {
        hdrlen = (0, utils_1.le_dword)(buffer, offset);
        this.instrument[i] = {};
        this.instrument[i].sample = [];
        this.instrument[i].name = "";
        j = 0;
        while (buffer[offset + 4 + j] && j < 22)
            this.instrument[i].name += (0, utils_1.dos2utf)(buffer[offset + 4 + j++]);
        this.instrument[i].samples = (0, utils_1.le_word)(buffer, offset + 27);
        // initialize to defaults
        this.instrument[i].samplemap = new Uint8Array(96);
        for (j = 0; j < 96; j++)
            this.instrument[i].samplemap[j] = 0;
        ...
        if (this.instrument[i].samples) {
            var smphdrlen = (0, utils_1.le_dword)(buffer, offset + 29);
            for (j = 0; j < 96; j++)
                this.instrument[i].samplemap[j] = buffer[offset + 33 + j];
            ...
            for (j = 0; j < 12; j++) {
                tmp_volenv[j] = new Uint16Array([
                    (0, utils_1.le_word)(buffer, offset + 129 + j * 4),
                    (0, utils_1.le_word)(buffer, offset + 129 + j * 4 + 2),
                ]);
                tmp_panenv[j] = new Uint16Array([
                    (0, utils_1.le_word)(buffer, offset + 177 + j * 4),
                    (0, utils_1.le_word)(buffer, offset + 177 + j * 4 + 2),
                ]);
            }
            ...
            // volume envelope
            for (j = 0; j < 325; j++)
                this.instrument[i].volenv[j] = 1.0;
            if (this.instrument[i].voltype & 1) {
                for (j = 0; j < 325; j++) {
                    var p, delta;
                    p = 1;
                    while (tmp_volenv[p][0] < j && p < 11)
                        p++;
                    if (tmp_volenv[p][0] === tmp_volenv[p - 1][0]) {
                        delta = 0;
                    }
                    else {
            ...
            // pan envelope
            for (j = 0; j < 325; j++)
                this.instrument[i].panenv[j] = 0.5;
            if (this.instrument[i].pantype & 1) {
                for (j = 0; j < 325; j++) {
                    var p, delta;
                    p = 1;
                    while (tmp_panenv[p][0] < j && p < 11)
                        p++;
                    if (tmp_panenv[p][0] === tmp_panenv[p - 1][0]) {
                        delta = 0;
                    }
                    else {
            ...
            // volume fade out
            this.instrument[i].volfadeout = (0, utils_1.le_word)(buffer, offset + 239);
            // sample headers
            offset += hdrlen;
            this.instrument[i].sample = new Array(this.instrument[i].samples);
            for (j = 0; j < this.instrument[i].samples; j++) {
                datalen = (0, utils_1.le_dword)(buffer, offset + 0);
                this.instrument[i].sample[j] = {};
                ...
                this.instrument[i].sample[j].loopstart = (0, utils_1.le_dword)(buffer, offset + 4) / this.instrument[i].sample[j].bps;
                this.instrument[i].sample[j].looplength = (0, utils_1.le_dword)(buffer, offset + 8) / this.instrument[i].sample[j].bps;
                this.instrument[i].sample[j].loopend =
                    this.instrument[i].sample[j].loopstart + this.instrument[i].sample[j].looplength;
                ...
                k = 0;
                this.instrument[i].sample[j].name = "";
                while (buffer[offset + 18 + k] && k < 22)
                    this.instrument[i].sample[j].name += (0, utils_1.dos2utf)(buffer[offset + 18 + k++]);
                offset += smphdrlen;
            }
            ...
            for (j = 0; j < this.instrument[i].samples; j++) {
                ...
                if (this.instrument[i].sample[j].bits === 16) {
                    for (k = 0; k < this.instrument[i].sample[j].length; k++) {
                        c += (0, utils_1.s_le_word)(buffer, offset + k * 2);
                        if (c < -32768)
                            c += 65536;
                        if (c > 32767)
                            c -= 65536;
                    ...
                    for (k = 0; k < this.instrument[i].sample[j].length; k++) {
                        c += (0, utils_1.s_byte)(buffer, offset + k);
                        if (c < -128)
                            c += 256;
                        if (c > 127)
                            c -= 256;
...
Fasttracker.prototype.process_note = function (mod, p, ch) {
    ...
    if (n < 254) {
        ...
        if (mod.channel[ch].noteon) {
            ...
            if (mod.pattern[p][pp + 1]) {
                // instrument set on row?
                mod.channel[ch].samplepos = 0;
                mod.channel[ch].playdir = 1;
                if (mod.channel[ch].vibratowave > 3)
                    mod.channel[ch].vibratopos = 0;
...
Fasttracker.prototype.process_tick = function (mod) {
    ...
    for (var ch = 0; ch < mod.channels; ch++) {
        ...
        if (v >= 0x50 && v < 0xf0) {
            if (!mod.tick)
                mod.voleffects_t0[(v >> 4) - 5](mod, ch, v & 0x0f);
            else
                mod.voleffects_t1[(v >> 4) - 5](mod, ch, v & 0x0f);
        }
        ...
        if (mod.instrument[i].voltype & 1) {
            ...
            if (mod.channel[ch].noteon &&
                ...
                mod.channel[ch].volenvpos = mod.instrument[i].volsustain;
            if (mod.instrument[i].voltype & 4 && mod.channel[ch].volenvpos >= mod.instrument[i].volloopend)
            ...
            if (mod.channel[ch].volenvpos > 324)
                mod.channel[ch].volenvpos = 324;
            ...
            if (!mod.channel[ch].noteon && mod.channel[ch].fadeoutpos) {
                ...
                if (mod.channel[ch].fadeoutpos < 0)
        ...
        if (mod.instrument[i].pantype & 1) {
            ...
            if (mod.channel[ch].noteon &&
                ...
                mod.channel[ch].panenvpos = mod.instrument[i].pansustain;
            if (mod.instrument[i].pantype & 4 && mod.channel[ch].panenvpos >= mod.instrument[i].panloopend)
            ...
            if (mod.channel[ch].panenvpos > 324)
                mod.channel[ch].panenvpos = 324;
        }
        // calc final volume for channel
        mod.channel[ch].finalvolume =
            (mod.channel[ch].voicevolume * mod.instrument[i].volenv[mod.channel[ch].volenvpos] * mod.channel[ch].fadeoutpos) /
                65536.0;
        // calc final panning for channel
        mod.finalpan[ch] =
            mod.pan[ch] +
                (mod.instrument[i].panenv[mod.channel[ch].panenvpos] - 0.5) * (0.5 * Math.abs(mod.pan[ch] - 0.5)) * 2.0;
...
Fasttracker.prototype.mix = function (mod, bufs, buflen) {
    ...
    if (mod.paused || mod.endofsong || !mod.playing) {
        for (var s = 0; s < buflen; s++) {
            bufs[0][s] = 0.0;
            bufs[1][s] = 0.0;
            for (var ch = 0; ch < mod.chvu.length; ch++)
                mod.chvu[ch] = 0.0;
        }
        return;
    }
        ...
        for (var ch = 0; ch < mod.channels; ch++) {
            ...
            if (mod.channel[ch].noteon ||
                ...
                if (mod.channel[ch].playdir === 1) {
                    if (Math.floor(mod.channel[ch].samplepos) > Math.floor(oldpos))
                        mod.channel[ch].lastsample = fs;
                }
                else {
                    if (Math.floor(mod.channel[ch].samplepos) < Math.floor(oldpos))
                        mod.channel[ch].lastsample = fs;
...
//
Fasttracker.prototype.effect_vol_t0_60 = function (mod, ch, data) {
    // 60-6f vol slide down
};
Fasttracker.prototype.effect_vol_t0_70 = function (mod, ch, data) {
    // 70-7f vol slide up
};
Fasttracker.prototype.effect_vol_t0_80 = function (mod, ch, data) {
    // 80-8f fine vol slide down
    mod.channel[ch].voicevolume -= data;
    if (mod.channel[ch].voicevolume < 0)
        mod.channel[ch].voicevolume = 0;
};
Fasttracker.prototype.effect_vol_t0_90 = function (mod, ch, data) {
    // 90-9f fine vol slide up
    mod.channel[ch].voicevolume += data;
    if (mod.channel[ch].voicevolume > 64)
        mod.channel[ch].voicevolume = 64;
};
Fasttracker.prototype.effect_vol_t0_a0 = function (mod, ch, data) {
    // a0-af set vibrato speed
    mod.channel[ch].vibratospeed = data;
};
Fasttracker.prototype.effect_vol_t0_b0 = function (mod, ch, data) {
    // b0-bf vibrato
    if (data)
        mod.channel[ch].vibratodepth = data;
    mod.effect_t1_4(mod, ch);
};
Fasttracker.prototype.effect_vol_t0_c0 = function (mod, ch, data) {
    // c0-cf set panning
    mod.pan[ch] = (data & 0x0f) / 15.0;
};
Fasttracker.prototype.effect_vol_t0_d0 = function (mod, ch, data) {
    // d0-df panning slide left
};
Fasttracker.prototype.effect_vol_t0_e0 = function (mod, ch, data) {
    // e0-ef panning slide right
};
Fasttracker.prototype.effect_vol_t0_f0 = function (mod, ch, data) {
    // f0-ff tone porta
    //  if (data) mod.channel[ch].slidetospeed=data;
    //  if (!mod.amigaperiods) mod.channel[ch].slidetospeed*=4;
};
//////
Fasttracker.prototype.effect_vol_t1_60 = function (mod, ch, data) {
    // 60-6f vol slide down
    mod.channel[ch].voicevolume -= data;
    if (mod.channel[ch].voicevolume < 0)
        mod.channel[ch].voicevolume = 0;
};
Fasttracker.prototype.effect_vol_t1_70 = function (mod, ch, data) {
    // 70-7f vol slide up
    mod.channel[ch].voicevolume += data;
    if (mod.channel[ch].voicevolume > 64)
        mod.channel[ch].voicevolume = 64;
};
Fasttracker.prototype.effect_vol_t1_80 = function (mod, ch, data) {
    // 80-8f fine vol slide down
};
Fasttracker.prototype.effect_vol_t1_90 = function (mod, ch, data) {
    // 90-9f fine vol slide up
};
Fasttracker.prototype.effect_vol_t1_a0 = function (mod, ch, data) {
    // a0-af set vibrato speed
};
Fasttracker.prototype.effect_vol_t1_b0 = function (mod, ch, data) {
    // b0-bf vibrato
    mod.effect_t1_4(mod, ch); // same as effect column vibrato on ticks 1+
};
Fasttracker.prototype.effect_vol_t1_c0 = function (mod, ch, data) {
    // c0-cf set panning
};
Fasttracker.prototype.effect_vol_t1_d0 = function (mod, ch, data) {
    // d0-df panning slide left
};
Fasttracker.prototype.effect_vol_t1_e0 = function (mod, ch, data) {
    // e0-ef panning slide right
};
Fasttracker.prototype.effect_vol_t1_f0 = function (mod, ch, data) {
    // f0-ff tone porta
...
//
Fasttracker.prototype.effect_t0_0 = function (mod, ch) {
    // 0 arpeggio
    mod.channel[ch].arpeggio = mod.channel[ch].data;
};
Fasttracker.prototype.effect_t0_1 = function (mod, ch) {
    // 1 slide up
    if (mod.channel[ch].data)
        mod.channel[ch].slideupspeed = mod.channel[ch].data * 4;
};
Fasttracker.prototype.effect_t0_2 = function (mod, ch) {
    // 2 slide down
    if (mod.channel[ch].data)
        mod.channel[ch].slidedownspeed = mod.channel[ch].data * 4;
};
Fasttracker.prototype.effect_t0_3 = function (mod, ch) {
    // 3 slide to note
    if (mod.channel[ch].data)
        mod.channel[ch].slidetospeed = mod.channel[ch].data * 4;
};
Fasttracker.prototype.effect_t0_4 = function (mod, ch) {
    // 4 vibrato
...
Fasttracker.prototype.effect_t0_5 = function (mod, ch) {
    // 5
    mod.effect_t0_a(mod, ch);
};
Fasttracker.prototype.effect_t0_6 = function (mod, ch) {
    // 6
    mod.effect_t0_a(mod, ch);
};
Fasttracker.prototype.effect_t0_7 = function (mod, ch) {
    // 7
};
Fasttracker.prototype.effect_t0_8 = function (mod, ch) {
    // 8 set panning
    mod.pan[ch] = mod.channel[ch].data / 255.0;
};
Fasttracker.prototype.effect_t0_9 = function (mod, ch) {
    // 9 set sample offset
...
Fasttracker.prototype.effect_t0_a = function (mod, ch) {
    // a volume slide
    // this behavior differs from protracker!! A00 will slide using previous non-zero parameter.
    if (mod.channel[ch].data)
        mod.channel[ch].volslide = mod.channel[ch].data;
};
Fasttracker.prototype.effect_t0_b = function (mod, ch) {
    // b pattern jump
    mod.breakrow = 0;
    mod.patternjump = mod.channel[ch].data;
    mod.flags |= 16;
};
Fasttracker.prototype.effect_t0_c = function (mod, ch) {
    // c set volume
...
Fasttracker.prototype.effect_t0_d = function (mod, ch) {
    // d pattern break
...
Fasttracker.prototype.effect_t0_e = function (mod, ch) {
    // e
    var i = (mod.channel[ch].data & 0xf0) >> 4;
    mod.effects_t0_e[i](mod, ch);
};
Fasttracker.prototype.effect_t0_f = function (mod, ch) {
    // f set speed
...
Fasttracker.prototype.effect_t0_g = function (mod, ch) {
    // g set global volume
    if (mod.channel[ch].data <= 0x40)
        mod.volume = mod.channel[ch].data;
};
Fasttracker.prototype.effect_t0_h = function (mod, ch) {
    // h global volume slide
    if (mod.channel[ch].data)
        mod.globalvolslide = mod.channel[ch].data;
};
Fasttracker.prototype.effect_t0_i = function (mod, ch) {
    // i
};
Fasttracker.prototype.effect_t0_j = function (mod, ch) {
    // j
};
Fasttracker.prototype.effect_t0_k = function (mod, ch) {
    // k key off
    mod.channel[ch].noteon = 0;
    if (!(mod.instrument[mod.channel[ch].instrument].voltype & 1))
        mod.channel[ch].voicevolume = 0;
};
Fasttracker.prototype.effect_t0_l = function (mod, ch) {
    // l set envelope position
    mod.channel[ch].volenvpos = mod.channel[ch].data;
    mod.channel[ch].panenvpos = mod.channel[ch].data;
};
Fasttracker.prototype.effect_t0_m = function (mod, ch) {
    // m
};
Fasttracker.prototype.effect_t0_n = function (mod, ch) {
    // n
};
Fasttracker.prototype.effect_t0_o = function (mod, ch) {
    // o
};
Fasttracker.prototype.effect_t0_p = function (mod, ch) {
    // p panning slide
};
Fasttracker.prototype.effect_t0_q = function (mod, ch) {
    // q
};
Fasttracker.prototype.effect_t0_r = function (mod, ch) {
    // r multi retrig note
};
Fasttracker.prototype.effect_t0_s = function (mod, ch) {
    // s
};
Fasttracker.prototype.effect_t0_t = function (mod, ch) {
    // t tremor
};
Fasttracker.prototype.effect_t0_u = function (mod, ch) {
    // u
};
Fasttracker.prototype.effect_t0_v = function (mod, ch) {
    // v
};
Fasttracker.prototype.effect_t0_w = function (mod, ch) {
    // w
};
Fasttracker.prototype.effect_t0_x = function (mod, ch) {
    // x extra fine porta up/down
};
Fasttracker.prototype.effect_t0_y = function (mod, ch) {
    // y
};
Fasttracker.prototype.effect_t0_z = function (mod, ch) {
    // z
};
//
// tick 0 effect e functions
//
Fasttracker.prototype.effect_t0_e0 = function (mod, ch) {
    // e0 filter on/off
};
Fasttracker.prototype.effect_t0_e1 = function (mod, ch) {
    // e1 fine slide up
    mod.channel[ch].period -= mod.channel[ch].data & 0x0f;
    if (mod.channel[ch].period < 113)
        mod.channel[ch].period = 113;
};
Fasttracker.prototype.effect_t0_e2 = function (mod, ch) {
    // e2 fine slide down
    mod.channel[ch].period += mod.channel[ch].data & 0x0f;
...
Fasttracker.prototype.effect_t0_e3 = function (mod, ch) {
    // e3 set glissando
};
Fasttracker.prototype.effect_t0_e4 = function (mod, ch) {
    // e4 set vibrato waveform
    mod.channel[ch].vibratowave = mod.channel[ch].data & 0x07;
};
Fasttracker.prototype.effect_t0_e5 = function (mod, ch) {
    // e5 set finetune
};
Fasttracker.prototype.effect_t0_e6 = function (mod, ch) {
    // e6 loop pattern
...
Fasttracker.prototype.effect_t0_e7 = function (mod, ch) {
    // e7
};
Fasttracker.prototype.effect_t0_e8 = function (mod, ch) {
    // e8, use for syncing
    mod.syncqueue.unshift(mod.channel[ch].data & 0x0f);
};
Fasttracker.prototype.effect_t0_e9 = function (mod, ch) {
    // e9
};
Fasttracker.prototype.effect_t0_ea = function (mod, ch) {
    // ea fine volslide up
    mod.channel[ch].voicevolume += mod.channel[ch].data & 0x0f;
    if (mod.channel[ch].voicevolume > 64)
        mod.channel[ch].voicevolume = 64;
};
Fasttracker.prototype.effect_t0_eb = function (mod, ch) {
    // eb fine volslide down
    mod.channel[ch].voicevolume -= mod.channel[ch].data & 0x0f;
    if (mod.channel[ch].voicevolume < 0)
        mod.channel[ch].voicevolume = 0;
};
Fasttracker.prototype.effect_t0_ec = function (mod, ch) {
    // ec
};
Fasttracker.prototype.effect_t0_ed = function (mod, ch) {
    // ed delay sample
    if (mod.tick === (mod.channel[ch].data & 0x0f)) {
        mod.process_note(mod, mod.patterntable[mod.position], ch);
    }
};
Fasttracker.prototype.effect_t0_ee = function (mod, ch) {
    // ee delay pattern
    mod.patterndelay = mod.channel[ch].data & 0x0f;
    mod.patternwait = 0;
};
Fasttracker.prototype.effect_t0_ef = function (mod, ch) {
    // ef
};
//
// tick 1+ effect functions
//
Fasttracker.prototype.effect_t1_0 = function (mod, ch) {
    // 0 arpeggio
    if (mod.channel[ch].data) {
        var i = mod.channel[ch].instrument;
...
Fasttracker.prototype.effect_t1_1 = function (mod, ch) {
    // 1 slide up
    mod.channel[ch].voiceperiod -= mod.channel[ch].slideupspeed;
    if (mod.channel[ch].voiceperiod < 1)
        mod.channel[ch].voiceperiod += 65535; // yeah, this is how it supposedly works in ft2...
    mod.channel[ch].flags |= 3; // recalc speed
};
Fasttracker.prototype.effect_t1_2 = function (mod, ch) {
    // 2 slide down
    mod.channel[ch].voiceperiod += mod.channel[ch].slidedownspeed;
    if (mod.channel[ch].voiceperiod > 7680)
        mod.channel[ch].voiceperiod = 7680;
    mod.channel[ch].flags |= 3; // recalc speed
};
Fasttracker.prototype.effect_t1_3 = function (mod, ch) {
    // 3 slide to note
...
Fasttracker.prototype.effect_t1_4 = function (mod, ch) {
    // 4 vibrato
...
Fasttracker.prototype.effect_t1_5 = function (mod, ch) {
    // 5 volslide + slide to note
    mod.effect_t1_3(mod, ch); // slide to note
    mod.effect_t1_a(mod, ch); // volslide
};
Fasttracker.prototype.effect_t1_6 = function (mod, ch) {
    // 6 volslide + vibrato
    mod.effect_t1_4(mod, ch); // vibrato
    mod.effect_t1_a(mod, ch); // volslide
};
Fasttracker.prototype.effect_t1_7 = function (mod, ch) {
    // 7
};
Fasttracker.prototype.effect_t1_8 = function (mod, ch) {
    // 8 unused
};
Fasttracker.prototype.effect_t1_9 = function (mod, ch) {
    // 9 set sample offset
};
Fasttracker.prototype.effect_t1_a = function (mod, ch) {
    // a volume slide
...
Fasttracker.prototype.effect_t1_b = function (mod, ch) {
    // b pattern jump
};
Fasttracker.prototype.effect_t1_c = function (mod, ch) {
    // c set volume
};
Fasttracker.prototype.effect_t1_d = function (mod, ch) {
    // d pattern break
};
Fasttracker.prototype.effect_t1_e = function (mod, ch) {
    // e
    var i = (mod.channel[ch].data & 0xf0) >> 4;
    mod.effects_t1_e[i](mod, ch);
};
Fasttracker.prototype.effect_t1_f = function (mod, ch) {
    // f
};
Fasttracker.prototype.effect_t1_g = function (mod, ch) {
    // g set global volume
};
Fasttracker.prototype.effect_t1_h = function (mod, ch) {
    // h global volume slude
...
Fasttracker.prototype.effect_t1_i = function (mod, ch) {
    // i
};
Fasttracker.prototype.effect_t1_j = function (mod, ch) {
    // j
};
Fasttracker.prototype.effect_t1_k = function (mod, ch) {
    // k key off
};
Fasttracker.prototype.effect_t1_l = function (mod, ch) {
    // l set envelope position
};
Fasttracker.prototype.effect_t1_m = function (mod, ch) {
    // m
};
Fasttracker.prototype.effect_t1_n = function (mod, ch) {
    // n
};
Fasttracker.prototype.effect_t1_o = function (mod, ch) {
    // o
};
Fasttracker.prototype.effect_t1_p = function (mod, ch) {
    // p panning slide
};
Fasttracker.prototype.effect_t1_q = function (mod, ch) {
    // q
};
Fasttracker.prototype.effect_t1_r = function (mod, ch) {
    // r multi retrig note
};
Fasttracker.prototype.effect_t1_s = function (mod, ch) {
    // s
};
Fasttracker.prototype.effect_t1_t = function (mod, ch) {
    // t tremor
};
Fasttracker.prototype.effect_t1_u = function (mod, ch) {
    // u
};
Fasttracker.prototype.effect_t1_v = function (mod, ch) {
    // v
};
Fasttracker.prototype.effect_t1_w = function (mod, ch) {
    // w
};
Fasttracker.prototype.effect_t1_x = function (mod, ch) {
    // x extra fine porta up/down
};
Fasttracker.prototype.effect_t1_y = function (mod, ch) {
    // y
};
Fasttracker.prototype.effect_t1_z = function (mod, ch) {
    // z
};
//
// tick 1+ effect e functions
//
Fasttracker.prototype.effect_t1_e0 = function (mod, ch) {
    // e0
};
Fasttracker.prototype.effect_t1_e1 = function (mod, ch) {
    // e1
};
Fasttracker.prototype.effect_t1_e2 = function (mod, ch) {
    // e2
};
Fasttracker.prototype.effect_t1_e3 = function (mod, ch) {
    // e3
};
Fasttracker.prototype.effect_t1_e4 = function (mod, ch) {
    // e4
};
Fasttracker.prototype.effect_t1_e5 = function (mod, ch) {
    // e5
};
Fasttracker.prototype.effect_t1_e6 = function (mod, ch) {
    // e6
};
Fasttracker.prototype.effect_t1_e7 = function (mod, ch) {
    // e7
};
Fasttracker.prototype.effect_t1_e8 = function (mod, ch) {
    // e8
};
Fasttracker.prototype.effect_t1_e9 = function (mod, ch) {
    // e9 retrig sample
...
Fasttracker.prototype.effect_t1_ea = function (mod, ch) {
    // ea
};
Fasttracker.prototype.effect_t1_eb = function (mod, ch) {
    // eb
};
Fasttracker.prototype.effect_t1_ec = function (mod, ch) {
    // ec cut sample
    if (mod.tick === (mod.channel[ch].data & 0x0f))
        mod.channel[ch].voicevolume = 0;
};
Fasttracker.prototype.effect_t1_ed = function (mod, ch) {
    // ed delay sample
    mod.effect_t0_ed(mod, ch);
};
Fasttracker.prototype.effect_t1_ee = function (mod, ch) {
    // ee
};
Fasttracker.prototype.effect_t1_ef = function (mod, ch) {
    // ef
};
