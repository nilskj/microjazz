...
*/
import { le_word, s_le_word, dos2utf, s_byte, le_dword } from "../helpers/utils";
    ...
    if (this.instrument[i].samples) {
      ...
      for (j = 0; j < this.instrument[i].samples; j++) {
        ...
        while (buffer[offset + 18 + k] && k < 22)
          this.instrument[i].sample[j].name += dos2utf(buffer[offset + 18 + k++]);
...
Fasttracker.prototype.effect_t1_0 = function (mod, ch) {
  ...
  if (mod.channel[ch].data) {
    ...
    mod.channel[ch].voiceperiod = mod.calcperiod(
      mod,
      apn + mod.instrument[i].sample[s].relativenote,
      mod.instrument[i].sample[s].finetune,
    );
    mod.channel[ch].flags |= 1;
  }
};
