/*
  ...
  - are Exx, Fxx and Gxx supposed to share a single
    command data memory?
*/
import { dos2utf } from "../helpers/utils";

export default function Screamtracker() {
  var i, t;
...
Screamtracker.prototype.parse = function (buffer) {
  ...
  for (i = 0; i < this.patNum; i++) {
    var offset =
      (buffer[0x0060 + this.ordNum + this.insNum * 2 + i * 2] |
      ...
      if ((c = buffer[offset + pos++])) {
        ...
        if (ch < this.channels) {
          ...
          if (c & 128) {
            ...
            if (
              !this.pattern[i][row * this.channels * 5 + ch * 5 + 3] ||
